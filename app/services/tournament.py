...
from app.schemas.tournament import TournamentCreate, TournamentRead, PlayerCreate, PlayerRead
...
async def tournament_create(db: DBSession, tournament: TournamentCreate) -> TournamentRead:
    ...
    overlapping = await db.execute(select(Tournament).where(
        Tournament.start_time.between(time_window_start, time_window_end)))
...
async def get_players_by_tournament(db: DBSession, tournament_id: int, ) -> list[PlayerRead]:
